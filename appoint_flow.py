...
def dr_advance_success_appointment(name,phone,payment_id,number):
    retrieved_data = appointment.find_one({"pay_id": payment_id})
    # dates = retrieved_data['date_of_appointment']
    fname = retrieved_data['guardian_name']
    fee = retrieved_data['amount']
...
# Query with date filtering (Convert date_of_appointment to datetime in query)
    ...
    if len(result)>0 and ap_type=='same':
        ...
        if slot=="current":
            ...
            ss = dr_advance_success_appointment(name,phone,pay_id,"918959690512")
            ss = dr_advance_success_appointment(name,phone,pay_id,"918128265003")
