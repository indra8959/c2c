...
from api_files.utils import new_hqz_transport_db
from bson.objectid import ObjectId
from datetime import datetime,timedelta
from zoneinfo import ZoneInfo
from api_files.new_hqz_transport.staff import staff_bp
from api_files.new_hqz_transport.trade import trade_bp
from api_files.new_hqz_transport.awsfile import aws_bp

new_hqz_transport = Blueprint("new_hqz_transport", __name__)





new_hqz_transport.register_blueprint(staff_bp, url_prefix="/staff")
new_hqz_transport.register_blueprint(trade_bp, url_prefix="/trade")
new_hqz_transport.register_blueprint(aws_bp, url_prefix="/aws")

vouchers = new_hqz_transport_db["vouchers"] 
groups_collection = new_hqz_transport_db["groups"] 
subgroups_collection = new_hqz_transport_db["subgroups"] 
ledgers_collection = new_hqz_transport_db["ledgers"] 


@new_hqz_transport.route("/payment_voucher", methods=["POST"])
def payment_voucher():
    try:
        data = request.json
...
@new_hqz_transport.route("/receipt_voucher", methods=["POST"])
def receipt_voucher():
    try:
        data = request.json
...
@new_hqz_transport.route("/journal_voucher", methods=["POST"])
def journal_voucher():
    try:
        data = request.json
...
@new_hqz_transport.route("/v1/vouchers", methods=["GET"])
...
@new_hqz_transport.route('/v1/ledger/<ledger_id>', methods=['GET'])
...
@new_hqz_transport.route('/v1/ledger2/<ledger_id>/<emp_name>', methods=['GET'])
...
# ------------------ GROUPS ------------------
@new_hqz_transport.route("/groups", methods=["POST"])
...
@new_hqz_transport.route("/groups", methods=["GET"])
def get_all_groups():
    try:
...
@new_hqz_transport.route("/subgroups", methods=["POST"])
...
@new_hqz_transport.route("/subgroups", methods=["GET"])
...
# ------------------ LEDGERS ------------------
@new_hqz_transport.route("/ledgers", methods=["POST"])
...
@new_hqz_transport.route("/ledgers", methods=["GET"])
def get_all_ledgers():
    try:
        pipeline = [
            {
...
@new_hqz_transport.route("/v1/financial-report", methods=["GET"])
...
@new_hqz_transport.route("/v1/financial-report-trading", methods=["GET"])
...
def get_closing_stock():
    try:
        ...
        # =========================
        vouchers = list(
            new_hqz_transport_db.vouchers.find(
                {"data": {"$exists": True}},
                {"data.items": 1, "date": 1}
            )
        )
        ...
        # =========================
        bills = list(
            new_hqz_transport_db.bills.find(
                {},
                {"items": 1}
            )
        )
...
@new_hqz_transport.route("/v1/financial-report-trial-balance", methods=["GET"])
